...
export function EvidenceCases({ chapter }: { chapter: string }) {
  ...
  return (
    <aside className="chapter-case-files">
      ...
      <div>{claims.map((claim) => <Link href={`/evidence/${claim.slug}`} data-state={claim.state} key={claim.slug}><span>{claim.number}</span><div><small>{claimStateLabel[claim.state]} · {claim.evidenceStatus}</small><strong>{claim.title}</strong></div><b>→</b></Link>)}</div>
    </aside>
  );
}

  ...
  return (
    <main id="main" className="chapter-page">
      ...
      <article className="chapter-body">
        ...
        {chapter.sections.map((section, sectionIndex) => <section key={section.heading}><div className="section-number">{String(sectionIndex + 1).padStart(2, "0")}</div><div><h2>{section.heading}</h2>{section.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}{index === section.paragraphs.length - 1 && sectionIndex === chapter.sections.length - 1 ? <sup><a href={`#source-${chapter.sourceRefs[0]}`}>1</a></sup> : null}</p>)}{section.evidence && <aside className="evidence-box"><p className="section-kicker">History / Memory / Evidence</p><h3>The familiar story</h3><p>{section.evidence.familiar}</p><h3>What the evidence shows</h3><p>{section.evidence.documented}</p>{section.evidence.uncertain && <><h3>What remains uncertain</h3><p>{section.evidence.uncertain}</p></>}</aside>}</div></section>)}
        ...
        {related.length > 0 && <section className="related-events"><p className="section-kicker">From the event corpus</p><h2>Events in this movement</h2><div>{related.map((event) => <article key={event.id}><time>{event.date}</time><h3>{event.title}</h3><p>{event.summary}</p></article>)}</div></section>}

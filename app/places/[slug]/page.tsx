...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const record = catalogRecord("places", slug); return record ? { title: record.name, description: record.summary } : {}; }

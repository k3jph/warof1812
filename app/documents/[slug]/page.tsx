...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const record = catalogRecord("documents", slug); return record ? pageMetadata(`/documents/${record.slug}`, record.name, record.summary) : {}; }

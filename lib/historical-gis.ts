...
export const gisSources: GisSource[] = [
  ...
  { id: "nhhc-flotilla", title: "Chesapeake Flotilla", institution: "Naval History and Heritage Command", url: "https://www.history.navy.mil/research/underwater-archaeology/sites-and-projects/ship-wrecksites/chesapeakeflotilla1812.html", note: "Barney's flotilla, Patuxent operations, and underwater archaeology." },
...
const defaultMethod = (title: string, geometry: GisGeometry, precision: GisFeature["properties"]["precision"]) => {
  if (geometry.type === "Point") return `${title} is placed at the cited named site or preservation location. The point identifies the record, not a survey-grade wartime coordinate.`;
  if (geometry.type === "Polygon") return `${title} is an interpretive area synthesized from the cited historical geography. Its edge is explanatory, not a surveyed or exclusive boundary.`;
  if (precision === "site") return `${title} connects a tightly located crossing or action from the cited place records. The line shows movement between known points rather than a surveyed track.`;
  return `${title} is drawn as a generalized corridor from the cited campaign and place records. Bends show sequence and orientation rather than a surveyed track.`;
};

const feature = (id: string, layer: GisLayerId, title: string, geometry: GisGeometry, start: string, end: string, summary: string, sourceIds: string[], confidence: GisFeature["properties"]["confidence"] = "medium", precision: GisFeature["properties"]["precision"] = "corridor", method?: string, extras: Pick<GisFeature["properties"], "then" | "now"> = {}): GisFeature => ({
  type: "Feature", id, geometry,
  properties: { title, layer, start, end, summary, sourceIds, confidence, precision, method: method ?? defaultMethod(title, geometry, precision), ...extras },
});
...
export const gisFeatures: GisFeature[] = [
  ...
  feature("washington-raid", "campaigns", "Benedict to Washington", { type: "LineString", coordinates: [[-76.69,38.51],[-76.64,38.75],[-76.86,38.94],[-77.04,38.9]] }, "1814-08-19", "1814-08-25", "Ross's force landed on the Patuxent, defeated the defense at Bladensburg, and entered Washington.", ["nhhc-flotilla","nps-war"], "high", "corridor"),
  ...
  feature("patuxent-flotilla", "logistics", "Chesapeake Flotilla corridor", { type: "LineString", coordinates: [[-76.4,38.3],[-76.6,38.55],[-76.68,38.75]] }, "1814-06-01", "1814-08-22", "Barney's shallow-draft flotilla maneuvered in the Patuxent before its destruction above Pig Point.", ["nhhc-flotilla"], "high", "corridor"),
...
export const historicalGeoJson = {
  ...
  name: "1812: The Whole Story: Campaign map GIS data",
